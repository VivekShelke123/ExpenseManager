...
const RecentExpense: React.FC<myprops> = (props) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:5000/getExpense/${userName}`)
      .then((res) => {
  ...
  function handleMap( obj : expenseDetails) {
    return (
      <div >
        <DemoPaper variant="outlined" style={{'display':'block'}}>
          <div className="expense-compo">
            <p style={{ display: "flex", justifyContent: "space-between" }}>
              <li>SpentOn : <li style={{'display':'inline'}}>{obj.spentOn}</li></li> <li>Amount : <li style={{'display':'inline'}}>{obj.amount}</li> </li>
            </p>
            <p style={{ display: "flex" }}>
              <li>Category : <li style={{'display':'inline'}}>{obj.category}</li></li>
  ...
  return (
    <div>
      <div className="title-class">
        <li style={{ fontSize: "2.2vw", marginBottom: "0%" }}>
          Recent Expense
        </li>
        <li style={{ fontSize: "0.99vw", marginTop: "0%" }}>
          Here are Your Recent Expense
        </li>
